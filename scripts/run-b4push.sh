...
# Pure-Node: reads src/ + zfb.config.ts only, no install or build needed.
# Guards against silently-dropped category metadata — zudo-doc's
# `loadCategoryMeta` cannot read `_category_.json` under zfb, so any sidecar
# is ignored without error, and a headerNav entry pointing at a
# `category_no_page: true` page 404s site-wide. Neither `zfb check`,
# html-validate, nor check-links can see either problem.
